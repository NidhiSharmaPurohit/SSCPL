...
import srmicrosystems.cnote.databinding.ActivityCnnoteKrishnaBinding;

//import srmicrosystems.cnote.databinding.ActivityCnnoteKrishnaBinding;

public class CNNoteKrishna extends AppCompatActivity {
    ...
    PaymentDetailsForCnote resp;
    PaymentCnnote paymentdetails;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_cnnote_krishna);
        ...
        ActivityCnnoteKrishnaBinding binding = DataBindingUtil.setContentView(this,R.layout.activity_cnnote_krishna);
        ...
        try{
            ...
            btnSubmit.setOnClickListener(new View.OnClickListener(){
                ...
                public void onClick(View v) {
                    ...
                    paysubmitdialog.setMessage("Submitting Krishna Information");
                    ...
                    try
                    {
                        dt = df1.format(resp.getBookingDate());
                    }
                    catch(Exception e)
                    {
                        //dt = new Date();
                        dt = resp.getBookingDate().toString();
                    }
                    ...
                    paymentdetails.Amount = resp.getAmount();
                    paymentdetails.BankName = bankname.getText().toString();
                    paymentdetails.CenterId = resp.getCenterID();
                    paymentdetails.ChequeNumber = chequenum.getText().toString();
                    paymentdetails.CNNumber = resp.getCNNumber();
                    paymentdetails.CreatedBy = paycreperson.getText().toString();
                    paymentdetails.CompanyId = resp.getShipperCompId();
                    ...
                    paymentdetails.InvoiceNum = "Invoice_" + resp.getCNNumber();
                    ...
                    paymentdetails.RateId = resp.getRateId();
                    paymentdetails.Statuss = 1;

                    paymentdetails.ToDate = dt;

                    paymentdetails.Weight = resp.getConsignmentWeight();
    ...
    public void Print(){
        pd.setMessage("Printing Krishna Receipt");
        pd.show();
        final BluetoothAdapter mAdapter = BluetoothAdapter.getDefaultAdapter();
        if (mAdapter == null) {
            pd.setMessage("Printing Krishna Receipt:Not Supported ");
        ...
        try {
            ...
            {   pd.setMessage("Printing Krishna Receipt:Connecting ");
                if ( mPrinter.initService(this, mMessenger) == true)
                {
                    if ( mPrinter.getState()!=CieBluetoothPrinter.STATE_CONNECTED){
                        pd.setMessage("Printing Krishna Receipt:Printer Connected: " + mPrinter.name() + " Battery status" + mPrinter.getBatteryStatus());
        ...
        mPrinter.printTextLine(" Receipt for AWB#:  " + resp.getCNNumber());
        mPrinter.printLineFeed();
        mPrinter.printTextLine(resp.getTransportMode() + "    ");
        // mPrinter.printTextLine("AIR              ");
        // mPrinter.printTextLine("SRT-BOM\n");
        mPrinter.printTextLine(resp.getSourceCityCode() + "-"+ resp.getDestCityCode() + "\n");
        ...
        mPrinter.printTextLine("   Date: "+ resp.getBookingDate().toString()  + "\n");
        ...
        mPrinter.printTextLine( resp.getPackageNo() + "\n");
        mPrinter.setRegular();
        ...
        mPrinter.setBold();
        //  mPrinter.printTextLine("100\n");
        mPrinter.printTextLine( resp.getConsignmentWeight() + "\n");
        mPrinter.setRegular();

        mPrinter.printTextLine(" Amount: ");
        mPrinter.setBold();
        mPrinter.printTextLine( String.valueOf(resp.getTotalAmount()) + " \n");
        mPrinter.printLineFeed();
        mPrinter.setRegular();
        ...
        //      }
        //  });
        pd.setMessage("Printing Krishna Receipt:Printing Done");
