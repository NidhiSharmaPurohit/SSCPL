...
namespace Consignment_Track_SSCPL
{
    public partial class InvoiceForm : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            PopulateTaxableamountGrid(total);


        }

        private void PopulateTaxableamountGrid(double total)
        {
            Tax[] result = CachedDataManager.GetCachedTax();
            List<TaxAmount> taxamountlist = new List<TaxAmount>();
            double totaltaxableamount = 0;
            foreach(Tax tx in result)
            {
                totaltaxableamount = totaltaxableamount + (total / 100) * tx.Percentage;
                taxamountlist.Add(new TaxAmount { TaxId = tx.TaxId, TaxName = tx.TaxName, Percentage = tx.Percentage, TaxAbleAmount = (total / 100) * tx.Percentage });
            }
            dataGridView2.DataSource = taxamountlist.ToArray<TaxAmount>();
            
            taxableamounttxt.Text = totaltaxableamount.ToString();
            finalamounttxt.Text = (total + totaltaxableamount).ToString(); 
        }
        ...
        private void DataGridView1_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            if (initial == false)
            {
                dataGridView1["Total", e.RowIndex].Value = (double)(dataGridView1["Rate", e.RowIndex].Value) * (double)(dataGridView1["ConsignmentWeight", e.RowIndex].Value);
            }
        }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            int numOfTaxRows = dataGridView2.Rows.Count;
            for (int row = 0; row < numOfTaxRows; row++)
            {
                //Inserting TaxInvoiceItems Now
                TaxInvoice taxinv = new TaxInvoice();
                taxinv.InvoiceNum = inv.InvoiceNum;
                taxinv.TaxId = (int)(dataGridView2["TaxId", row].Value);
                taxinv.TaxAmount = (double)(dataGridView2["TaxAbleAmount", row].Value);

                new DataManager().AddTaxInvoiceItemRecord(taxinv);
                

            }

            InvoiceDisplayAndPrint frm = new InvoiceDisplayAndPrint();
